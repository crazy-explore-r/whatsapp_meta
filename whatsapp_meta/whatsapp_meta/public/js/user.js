...
function set_primary_action(frm){
  var feild_dicts = [];
  frappe.db.get_single_value("WhatsApp Settings", "welcome_message_template").then( whatsapp_message_template => {
    frappe.call({
      method: 'whatsapp_meta.whatsapp_meta.doctype.whatsapp_message_template.whatsapp_message_template.set_template_parameters',
      args: {
...
function send_welcome_message(frm, values){
  frappe.call({
    method: 'whatsapp_meta.whatsapp_meta.utils.send_welcome_message',
    args: {
